...
def recommend(session_list, playlist):
    """
        Return ranked phrases using MMR. Cosine similarity is used as similarity measure.
        :param session_list: Songs that the user has listened until now
        :param playlist: Songs to rank
        """
    result = np.zeros(len(session_list))
    for i in session_list:
        print(i)
        result = result + session_list[i]
